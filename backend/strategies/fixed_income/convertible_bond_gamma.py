...
def run(cfg):
    ...
    if not gamma_df.empty:
        for ticker in gamma_df["ticker"].unique():
            ...
            delta_series = sub["delta"].reindex(stock_ret.index, method="ffill").shift(1)
            gamma_series = sub["gamma"].reindex(stock_ret.index, method="ffill").shift(1)
            ...
            hedge_ratio = sub["delta"].reindex(stock_ret.index, method="ffill").shift(1).fillna(0)
    ...
    print(f"CB gamma | Tickers: {summary['n_tickers']} | Buy vol: {summary['n_buy_signals']} | Sell vol: {summary['n_sell_signals']} | Sharpe: {sharpe:.2f if sharpe else 'N/A'} | Written to {cfg.outdir}")
